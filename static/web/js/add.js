$(function () {
    ...
    $(".add").click(function(){
        ...
        let m=parent.parent().children("h4").text(); //当前商品名称
        let danjia=$(this).next().text(); //获取单价
	    let a = $("#totalpriceshow").html();  //获取当前所选总价
	    $("#totalpriceshow").html((a * 1 + danjia * 1).toFixed(2));//计算当前所选总价
	    let nm = $("#totalcountshow").html(); //获取数量
	    $("#totalcountshow").html(nm*1+1);
	    jss();   //改变按钮样式

	    let acount =num;
	    let sum = (danjia*acount).toFixed(2);
	    let price =danjia;
		//判断购物车里是否有商品，是否有相同规格的商品
		if($(".list-content ul li").length <= 0){
			let addtr = '<li class="food">';
			...
			$(".list-content ul li").each(function(){
				if ($(this).find("span.accountName").html() == m) {
					let count = parseInt($(this).find(".li_acount").html());
					count = parseInt(num);
	...
	$(document).on('click','.ad2',function(){
		let n = parseInt($(this).prev().text())+1;
		$(this).prev().text(n);    //当前商品数量+1
		e.text(n);    //赋值给商品列表的数量
		let p = parseFloat($(this).next().text());    //隐藏的价格
	...
	$('.list-content').on('click','.ms2',function(){
		let e;
		let m = $(this).parent().parent().find(".accountName").text();  //当前商品名字
		let a = parseFloat($(this).siblings(".price").text());  //当前商品单价
		let n = parseInt($(this).next().text())-1;  //当前商品数量
		let s = parseFloat($("#totalpriceshow").text());  //总金额
		let znum=0;
    ...
    $(".footer>.left").click(function(){
        let content = $(".list-content>ul").html();
        if(content!=""){
